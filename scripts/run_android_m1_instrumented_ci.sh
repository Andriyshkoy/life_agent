...
readonly PROJECT_DIR="${M1_PROJECT_DIR:-/workspace}"
readonly DIAGNOSTICS_DIR="$PROJECT_DIR/app/build/outputs/m1-cold-start"
...
if [[ "$device_api" != "35" ]]; then
    printf 'M1 instrumented gate requires API 35, got %s.\n' "$device_api" >&2
    exit 1
fi
if [[ ",$device_abis," != *,x86_64,* ]]; then
    printf 'M1 instrumented gate requires x86_64, got %s.\n' "$device_abis" >&2
    exit 1
fi
