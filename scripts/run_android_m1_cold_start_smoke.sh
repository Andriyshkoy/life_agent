...
usage() {
    cat <<'EOF'
Usage:
  run_android_m1_cold_start_smoke.sh --destructive-local-test DEBUG_APK TEST_APK

Runs the M1 airplane-mode + force-stop acceptance on the fixed debug package
...
if [[ "$device_api" != "35" ]]; then
    printf 'M1 cold-start gate requires Android API 35; device reports %s.\n' \
...
if [[ ",$device_abis," != *,x86_64,* ]]; then
    printf 'M1 cold-start gate requires x86_64; device reports %s.\n' \
        "${device_abis:-unknown}" >&2
    exit 65
fi

readonly SYNTHETIC_MARKER="m1-cold-$(date -u +%Y%m%dT%H%M%SZ)-$$"
...
run_phase() {
    ...
    if ! output=$(
        timeout --foreground "$INSTRUMENTATION_TIMEOUT" \
            ...
            -e m1ColdStartMarker "$SYNTHETIC_MARKER" \
            -e m1RequireAirplaneMode true \
            "$TEST_RUNNER" 2>&1
    ); then
        printf '%s\n' "$output" >&2
        return 1
...
printf 'PASS: airplane-mode UI seed, force-stop, cold-launch UI, SQLCipher note and outbox.\n'
