...
interface WellbeingQueryDao {
    ...
    @Query(
        ...
        WHERE e.event_id = :eventId AND e.kind = 'wellbeing'
        """,
    )
    suspend fun findCurrent(eventId: String): CurrentRevisionRow?
    ...
    @Query(
        ...
        JOIN local_capture AS c
          ON c.capture_id = r.capture_id AND c.operation_id = r.operation_id
        JOIN local_life_event AS e ON e.event_id = r.event_id
        WHERE r.operation_id = :operationId AND e.kind = 'wellbeing'
        """,
    )
    ...
    @Query(
        ...
        WHERE kind = 'wellbeing'
        ORDER BY event_id
        """,
    )
    suspend fun findAllEventIds(): List<String>
